...
public class JODConverter {
    ...
    public synchronized void convertToPDF(File archivoDocx, File archivoPDF)  {
        LOGGER.info("Convirtiendo documento");
        LOGGER.info("Convirtiendo documento archivoDocx -->" + archivoDocx.getPath());
        LOGGER.info("Convirtiendo documento archivoPDF -->" + archivoPDF.getPath());
            
        OfficeDocumentConverter converter = new OfficeDocumentConverter(officeManager);
        LOGGER.info("new OfficeDocumentConverter(officeManager);");
       
        LOGGER.info("converter.convert(archivoDocx, archivoPDF)");
        converter.convert(archivoDocx, archivoPDF);
        LOGGER.error("converter.convert(archivoDocx, archivoPDF) HECHO!!!!");
    }
    ...
    @PostConstruct
    public void start() {
        LOGGER.info("INICIA EL SERVICIO DE LIBRE OFFICE CON UNA INSTANCIA YA EXISTENTE");
        try {
            ...
            this.officeManager =
                    new DefaultOfficeManagerConfiguration().setPortNumbers(PUERTO_DEFAULT, PUERTO_ALTERNATIVO1,
                                                                           PUERTO_ALTERNATIVO2, PUERTO_ALTERNATIVO3,
                                                                           PUERTO_ALTERNATIVO4).buildOfficeManager();
            this.officeManager.start();
    ...
    @PreDestroy
    public void stop() {
        LOGGER.info("PARA EL SERVICIO DE LIBRE OFFICE CON UNA INSTANCIA YA EXISTENTE");
        officeManager.stop();
    }
